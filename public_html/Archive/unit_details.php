...
<title>Jackson County Sheriff Auxiliary - Unit Details</title></head>
...
if($level== '2' || $level == '5' || $level == '6' || $level == '9') {
	echo "<br/><br/><a href=\"adddetail.php\"><img 
src=\"images/add_detail.jpg\"></a><br/><br/>"; }

echo "<h1>Jackson County Sheriff's Auxilliary Details as of $today.</h1>";
echo "<h3>Click on the checkmark to volunteer for a particular 
detail.</h3>"
echo "<h3> Once you volunteer, you do not have the ability to 
unvolunteer. You must contact the detail coordinator to have your name 
removed.</h3>";
...
?>
<table width=1050 border=1 align="left" cellpadding=2 
cellspacing=1>

<CAPTION><EM><h2>Active Details</h2></EM></CAPTION>
<tr><th width="25"></th><th width="25"></th><th width=25px></th><th width=25px></th><th bgcolor="gray" 
width=80px><b>Date</b></th><th bgcolor="gray" 
width=170px><b>Location</b></th><th bgcolor="gray" width=145px><b>Type 
of 
Function</b></th><th bgcolor="gray" width="50"><b>Start 
Time</b></th><th 
bgcolor="gray" width="50"><b>End 
Time</b></th><th bgcolor="gray" width="160px"><b>Contact 
Person</b></th><th bgcolor="gray" width="50px"<b>Number Officers<th 
bgcolor="gray" width="150px"><b>Officer 
Assigned</b></th><th bgcolor="gray" width="40"><b>Pay</b></th></tr>
...
if(($detail_date - $today_number) < (43*60*60*24)) {  
...
/*  The detail is in the past and we have not processed the detail sheet  */
	 if($todays_date > $date) {
		...
		print("<tr>$post $edit<td></td><td bgcolor=$color>$date</td><td bgcolor=$color>$location</td><td bgcolor=$color>$type</td><td bgcolor=$color>$start_time</td><td 
...
bgcolor=$color>$blank</td><td bgcolor=$color>$blank</td><td 
bgcolor=$color>$blank</td><td 
bgcolor=$color>$blank</td><td bgcolor=$color>$blank</td><td 
bgcolor=$color>$blank</td><td bgcolor=$color>$blank</td><td bgcolor=$color>$officer[$z]</td><td bgcolor=$color>$blank</td></tr>\r");
					}
			}

/*  The detail is not filled and we need to show the volunteer checkmark  */
 	} else if($num_assigned < $num_officers) {
		$color="gold";
...
/></a></td><td bgcolor=$color>$date</td><td bgcolor=$color>$location</td><td bgcolor=$color>$type</td><td 
bgcolor=$color>$start_time</td><td bgcolor=$color>$end_time</td><td bgcolor=$color>$contact</td><td bgcolor=$color align=\"center\">$num_officers</td><td 
bgcolor=$color>$officer[0]</td><td bgcolor=$color>$pay</td></tr>\r");
		}
		...
		if($num_assigned > 1)	{
			for ($z=1; $z<$num_assigned; $z++)	{
				print("<tr><td></td><td></td><td></td><td></td><td bgcolor=$color>$blank</td><td bgcolor=$color>$blank</td><td bgcolor=$color>$blank</td><td bgcolor=$color>$blank</td><td 
bgcolor=$color>$blank</td><td bgcolor=$color>$blank</td><td bgcolor=$color>$blank</td><td bgcolor=$color>$officer[$z]</td><td bgcolor=$color>$blank</td></tr>\r");
					}
		}
...
href=\"volunteer.php?type=delete&detail=$detail\"><img src=\"images/delete.jpg\" /></a></td>";	}
		...
		print("<tr>$post $edit<td></td><td bgcolor=$color>$date</td><td bgcolor=$color>$location</td><td bgcolor=$color>$type</td><td 
bgcolor=$color>$start_time</td><td 
bgcolor=$color>$end_time</td><td bgcolor=$color>$contact</td><td 
bgcolor=$color align=\"center\">$num_officers</td><td 
bgcolor=$color>$officer[0]</td><td 
bgcolor=$color>$pay</td></tr>\r");
...
bgcolor=$color>$blank</td><td bgcolor=$color>$blank</td><td 
bgcolor=$color>$blank</td><td 
bgcolor=$color>$officer[$z]</td><td 
bgcolor=$color>$blank</td></tr>\r");
					}
			}
	}
}
