...
$q = "SELECT f_name,  l_name, uid FROM ".TBL_USERS." ORDER BY l_name";
...
/* Query the credits database for each user except 'visitor' */
	$q = "SELECT SUM(money_made) FROM credits WHERE credits.date_posted > '2019-12-01' AND  uid = '" . $uid . "'";
	$credits = $database->query($q);

	/* Check for Errors in Reading Table */
	$credit_rows = mysql_num_rows($credits);
...
/*			$total_credits = $total_credits + $current_credit;
		 ...
		 */
//
		$q = "SELECT SUM(amount) FROM expenses WHERE expenses.date > '2019-12-02' AND uid = '" . $uid . "'";
		$expenditures = $database->query($q);

		/* Check for Errors in Reading Table */
		$expenditures_rows = mysql_num_rows($expenditures);
...
//		$total_owed = number_format($total_owed,2);
		...
		if($current_credit == 0.00)	{
			if($current_expenses == 0.00)	{
			continue;
			/*$deputy[] = "<tr><td>$name</td><td align=\"right\">0.01</td><td 
align=\"right\">0.01</td><td align=\"right\">$current_balance</td><tr>\r";*/
			} else {
				$deputy[] = "<tr><td>$name</td><td 
align=\"right\">0.00</td><td align=\"right\">$current_expenses</td><td align=\"right\">$current_balance</td><tr>\r";
			}
		} elseif ($current_expenses == 0.00)	{
				$deputy[] = "<tr><td>$name</td><td 
...
?>
<tr>
	<td colspan="3" align="right"><b>Total Owed by Auxiliary section to Deputies</b></td>
