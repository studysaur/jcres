...
unvolunteer. You must contact the detail coordinator (Tommy Miller) or the Auxiliary Deputy Chief (Jeff Mattison) to have your name removed.</h2>";
...
// echo "<h1>$lastname</h1>";
if ($edet || $pdet) {
    $q = "SELECT * FROM ".TBL_DETAILS." WHERE sheetPosted='0' ORDER BY date, startTime";
    } else {
    $q = "SELECT * FROM ".TBL_DETAILS . " WHERE sheetPosted = '0' and contact != 'reuse' ORDER BY date, startTime";
    }
...
//  	$volu="<td><a href=\"volunteer.php?type=volunteer&detail=$detail&field=$num_assigned&location=$location\"><img src=\"images/checkmark.jpg\" /></a></td>";
    ...
    if($type == "Tournament") {//look for school tournaments to set up locations
 	...
 	if($type =="Chevron Traffic"){// look for Chevron Traffic detail to set up posts
	    $location = "Hwy 90 At Chevron Dr";// make this the post assignment on the first line
	}
//	$loc="<td></td>";	
		...
		if($num_assigned > 1)	{
			for ($z=1; $z<$num_assigned; $z++)	{
			    if($type =="Chevron Traffic"){
				    ...
				    if($z==2){
					    $location="Hwy 11, North Parking lot exit";
				    }
				    if($z==3){
					    $location="Hwy 11 at Heavy Haul Rd.  Monitor stop sign";
				    }
			    } else {
			        $location = "";
			    }
 //print("<tr class=\"$cl\"><td></td><td></td><td></td><td></td><td></td><td></td><td>$location</td><td></td><td></td><td></td><td> $loc</td><td></td><td>$officer[$z]</td><td></td></tr>\r");
			}
		}
	}
