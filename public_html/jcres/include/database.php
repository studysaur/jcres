...
//      $user = mysql_fetch_object($result, 'user');
    ...
    * (1 or 2). On success it returns 0.
    */
   function confirmUserID($username, $password){ // the password here is encrypted changed from Userid to status
      /* Add slashes if necessary (for query) */
      if(!get_magic_quotes_gpc()) {
	      $username = addslashes($username);
      }
      ...
      /* Verify that user is in database */
      $q = "SELECT uid, status, password FROM ".TBL_USERS." WHERE username = '$username'";
      $result = mysql_query($q, $this->connection);
      if(!$result || (mysql_num_rows($result) < 1)){
         return 1; //Indicates username failure
      }
      ...
      $dbarray = mysql_fetch_array($result);
      $uid = stripslashes($dbarray['uid']);
      $status = $dbarray['status'];
      $uid = stripslashes($uid);

      /* Validate that uid is correct */
      if($password == $dbarray['password']){
     	  $_SESSION['uid'] = $uid;
...
paidDetail='$paid_detail' WHERE detailNum = '$detail_num'";
    ...
    *  into the correct record in the details database.
    */
   function updateUserAccountByAdmin($subuser, $subfname, $submname, $sublname, $subunit_num, $subrank, $subcode, $subsquad, $subphone_home, $subphone_work, $subphone_cell, $subuid)	{
