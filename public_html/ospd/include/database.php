...
//      $user = mysql_fetch_object($result, 'user');
	  ...
	  if($password == $default_password) {
       ...
       if($dbarray['status'] & 2147483648) {
          ...
          $_SESSION['username'] = $username; //Set the Session variables
          $_SESSION['password'] = $password; //to be used in Session
    ...
    * (1 or 2). On success it returns 0.
    */
   function confirmUserID($username, $password){ // the password here is encrypted
      /* Add slashes if necessary (for query) */
      if(!get_magic_quotes_gpc()) {
	      $username = addslashes($username);
      }
      ...
      /* Verify that user is in database */
      $q = "SELECT status, password FROM ".TBL_USERS." WHERE username = '$username'";
      $result = mysql_query($q, $this->connection);
      if(!$result || (mysql_num_rows($result) < 1)){
         return 1; //Indicates username failure
      }
...
/*   function usernameBanned($username){
  ...
  */
    ...
    */
   function addNewDetail($date, $detail_type, $detail_location, $start_time, $end_time, 
   $contact, $num_officers, $officer_1, $officer_2,  $officer_3,  $officer_4,  $officer_5,  $officer_6,  $officer_7,  $officer_8,  $officer_9,  $officer_10, $paid_detail)	{
   ...
   /*$q = "INSERT INTO reserves.".TBL_DETAILS." (`date`, `detailType`, `detailLocation`, `startTime`, `endTime`, `contact`, `contactPh`, `numOfficers`, `officer_1`, `officer_2`, `officer_3`, `officer_4`, `officer_5`, `officer_6`, `officer_7`, `officer_8`, `officer_9`, `officer_10`, `sheetPosted`, `paidDetail`)
    VALUES ('$date', '$detail_type', '$detail_location', '$start_time', '$end_time', '$contact', '', '$num_officers', '$officer_1', '$officer_2', '$officer_3', '$officer_4', '$officer_5', '$officer_6', '$officer_7', '$officer_8', '$officer_9', '$officer_10', 0, '$paid_detail')"; */
  // return $q;
    return mysql_query($q, $this->connection);
   }
