...
$q = "SELECT * FROM user INNER JOIN squads ON user.squad = squads.squad ORDER BY sid, FIELD(user.rank, 'Sheriff', 'Chief', 'Deputy Chief', 'Major', 'Captain', 'Chaplin', 'Lieutenant', 'Sergeant', 'Deputy'), unit_num";
$r = $pdo->query($q);
...
// echo 'fetching';
{
	...
	$fname = $user->f_name;
	$lname = $user->l_name;
	$phome = $user->phone_home;
//	$fsquad = $user->squad;
//	if (!$pwork){$pwork=chr(0);}
	$pcell = $user->phone_cell;
	if (!$pcell){$cell=chr(0);}	
	/*$probationary = $user->isUser(Aux\Entity\User::PROBATION);
